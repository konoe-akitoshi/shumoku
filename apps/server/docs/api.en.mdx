...
secret-authenticated webhook ingress require authentication and route-specific permissions.
...
See the [authentication guide](api-access.guide.en.md) and [SSO installation guide](installation.guide.en.md) for proxy mode, role mapping, and recovery. Proxy mode disables local login/setup/password changes and reports `authMethod: proxy`.
...
accepts local sessions or opt-in proxy authentication; proxy mode ignores this development credential. The credential is stored in a
...
`workspace:read` permission through a local session, proxy identity, or development Bearer credential:
...
| GET | `/api/topologies/:id/export?format=svg\|png\|html` | Export the topology |
| GET | `/api/topologies/:id/sources` | List linked data sources |
| POST | `/api/topologies/:id/sources` | Link a data source |

The export response is returned as an attachment. SVG and PNG export the requested
`sheet` (the root sheet by default); PNG also accepts a `scale` from `0.25` to `4`.
HTML is a standalone interactive document and includes navigation for every sheet.
...
`viewer`, `user`, or `admin`), `authMethod`, and `permissions` in addition to the
compatibility `authenticated` flag. The deprecated `publicDemo` field remains in
the response for API compatibility and is always `false`; it no longer enables
or reports an authentication mode. Anonymous requests remain anonymous and
cannot access workspace or management APIs. An authenticated `viewer` can
read workspace resources but cannot mutate them or access administrator surfaces.
...
and other authenticated identities with `workspace:read` can receive metrics.
The server validates browser Origin and resolves a local session or trusted proxy
identity using the same policy as HTTP. Existing sockets retain their principal
until disconnected. Oversized messages are rejected.
