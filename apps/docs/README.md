...
`SiteIdentity.astro` is shared by content, redirect, language-entry and recovery pages.
The Astro configuration copies the six canonical icons from `/assets` into `public/`
at startup/build; those generated copies are ignored by Git. Update `/assets`, not the copies.
`site.webmanifest.ts` emits Docs-specific metadata in browser mode (no service worker or
offline/install behavior). `DocsLayout.astro` owns localized Open Graph/Twitter metadata,
while existing canonical, language alternates, Markdown discovery and JSON-LD remain intact.
No social-preview artwork is generated by this integration.

Every build runs `scripts/check-site-identity.ts` to verify the emitted icons match the
canonical sources, manifest references exist, and all HTML pages include shared identity.
