...
- [ ] **C5 二段構えにしない(決定5)**: access は **1プロトコル=1行の単一の有効値**として
      常に編集可能に出す。provenance は「この値の出どころ(scan / 人)」の**注記**として添える
      だけで、読み取り専用の層を作らない。編集＝最優先の上書き、クリア/Revert＝上書き解除で
      観測値/既定へ戻す。観測値そのものは事実なので「削除」でなく Hide で対応。(web helper
      unit test + provenance unit test)
...
### 決定5: provenance はフィールド/attachment 単位の「注記」（層にしない）
...
常に編集可能に出し、provenance はその値の出どころ表示に使うだけ**。編集すれば人が最優先で
上書きされ、クリアすれば上書きが外れて下位ソースの値に戻る。「✕で observed が消えない」は
「観測値は事実なので削除でなく上書き/Revert/Hide」という一本化された操作で解消する。
（初回実装で決定5を「観測=読み取り専用＋Override ボタン」と取り違え、二段構えを UI に
復活させてしまった。provenance=注記であって層ではない、を厳守する。）
...
- **Access 行（community 等）** = 1プロトコル=1行の**単一の有効値**を常に編集可能に出す。
  入力を変えれば人の上書き（最優先）、空にすれば上書き解除でソース値へ戻る。人が上書き中の
  行には「観測値に戻す(Revert)」、純粋に人が足した行（観測値なし）には「削除」を出す。
  observed の事実（scan が public で読めている）は人が消すものではないので、その行に
  「削除」は出さない（編集して上書きするか、ノードごと Hide）。`provenance` は「あなたの値 /
  scan が取得」の注記表示に使うだけで、読み取り専用の層は作らない。
