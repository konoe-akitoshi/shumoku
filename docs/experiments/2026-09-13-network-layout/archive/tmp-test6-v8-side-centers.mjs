...
const source = 'tmp-test6-v7-direct-routing'
const output = 'tmp-test6-v8-side-centers'
...
const checks = {
  ...
  exteriorChanged: 0,
  missingBoundary: 0,
  nonMidpointAttachments: 0,
}
let clearConnections = 0,
  previousClearButBent = 0
for (const [i, r] of routes.entries()) {
  for (const [n, p] of [
    [links[i].a, r.points[0]],
    [links[i].b, r.points.at(-1)],
  ]) {
    const v = nodes[n],
      dx = Math.abs(p.x - v.x),
      dy = Math.abs(p.y - v.y)
    if (
      !(
        (Math.abs(dx - v.w / 2) < 1e-6 && dy < 1e-6) ||
        (dx < 1e-6 && Math.abs(dy - v.h / 2) < 1e-6)
      )
    )
      checks.nonMidpointAttachments++
  }
  ...
  if (JSON.stringify(ps) !== JSON.stringify(links[i].points.slice(1, -1))) checks.exteriorChanged++
}
...
const svg = oldSvg
  .replace(
    'V7 · Direct boundary routing / unchanged placement',
    'V8 · Node side-center attachments / unchanged placement',
  )
...
const report = {
  ...
  routing: {
    model:
      'Node endpoints use exact side midpoints, choosing the side hit by the ray toward the boundary terminal (external) or other node center (internal). Horizontal side wins exact corner ties. Placement, boundary terminals and exterior routes unchanged. No normal stubs; multiple links may share a midpoint.',
    clearConnections,
    previousClearButBent,
    checks,
    before: connectionMetrics(links, nodes, terminals, links),
    after: connectionMetrics(newLinks, nodes, terminals, newLinks),
    renderedLengthBefore: links.reduce((s, l) => s + length(l.points), 0),
    renderedLengthAfter: routes.reduce((s, l) => s + length(l.points), 0),
  },
}
...
const comparison = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w * 2 + 24} ${h + 38}" font-family="Segoe UI,sans-serif"><rect width="100%" height="100%" fill="#f7f9fc"/><text x="12" y="24" fill="#18334d" font-size="18">V7: ray / node boundary intersection</text><text x="${w + 36}" y="24" fill="#18334d" font-size="18">V8: exact side-center attachments</text>${panels.join('')}</svg>`
