...
cannot access workspace or management APIs. A future authenticated `viewer` can
...
and other authenticated sessions with `workspace:read` can receive metrics.
The server validates browser Origin, requires an authenticated session, and
