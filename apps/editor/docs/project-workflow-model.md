...
- `Materials`: Node への機材割り当てと、このプロジェクトで使う機材・部材の定義
...
| Materials | Node に対して機材、SKU、型番をまとめて割り当てたい | Node に Product を割り当て、必要なら Product を登録する |
...
flowchart LR
  ...
  A[Materials / Assignments<br/>Node に機材を割り当て]
  D[Diagram<br/>Node 候補として使う]
  C[Connections<br/>module / cable 条件として使う]
  B[BOM<br/>SKU 単位で集計]
...
Materials / Assignments では、現行 BOM が持っていた「Node と機材の紐づけ」を表で編集します。これは KiCad の symbol instance と footprint / part を対応づける操作に近いです。module / cable は接続仕様の一部なので、Materials へ寄せず Connections で編集します。
...
flowchart TB
  ...
  Materials[Materials<br/>Assignments / Library]
  Assignments[Assignments<br/>Node ↔ Product]
...
トップレベルは `Diagram` / `Connections` / `Materials` / `BOM` の 4 ページにします。`Materials` は Node への機材割り当てと、このプロジェクトで使う機材・部材を扱う workbench で、内部に `Assignments` / `Library` を持ちます。`BOM` は Materials の下位ではなく、設計全体から導出される確認ページとして独立させます。
...
- `Materials / Assignments` は Node に対する Product 割り当てをまとめて編集する。
...
| Materials / Assignments | `BomItem[]`, `Node.productId` | Node と Product の割り当て表 |
...
初期実装では `Inventory` を保存モデルに入れません。ただし将来足すときに壊れないよう、`Product.id` と図面上の参照先を安定した参照点として設計します。
...
flowchart TB
  ...
  subgraph Design[Design layer]
    G[NetworkGraph<br/>Node / Port / Link]
    A[DeviceAssignment<br/>機材行 ↔ Node]
  ...
  subgraph Future[Future inventory layer]
    ...
    AL[Allocation<br/>InventoryItem ↔ placement target]
  end
...
実線が初期実装、点線が将来の在庫管理です。初期実装では図面上に置かれたものは `Node` / `Link` として存在し、Product への参照は optional です。将来 `InventoryItem` を追加する場合も、図面を資産台帳に変えるのではなく、`InventoryItem` が `Product` と placement target を参照します。
...
| Device Assignment | Node と機材の割り当て | `BomItem[]`、bound Node の `productId` | module / cable の接続仕様 |
...
Device Assignment は現行 BOM が持っていた「Node と機材の対応」を正しい名前で扱う層です。ここが KiCad の schematic symbol と footprint / part を対応づける操作に相当します。
...
実装上の既存名は `BomItem` ですが、意味としては BOM ではなく device assignment row です。Product を選ぶと `productId` と Node の spec snapshot を更新し、Node を選ぶと機材行と diagram node を bind します。

module / cable はこの表へ入れません。module / cable は Link の接続仕様に属するため Connections で編集し、BOM では requirements として導出します。将来の在庫管理で SFP や cable 個体を持つ場合は、`node`、`link-cable`、`link-module` のような placement target を別途導入します。

### Future Inventory

在庫管理を追加する場合も、初期モデルの責務は変えません。追加するのは `Product` と placement target を参照する別レイヤーです。
...
Product       = 型
Node / Link   = 設計インスタンス
Assignment    = 初期実装では Node と機材の対応
InventoryItem = 実物または在庫単位
Allocation    = 実物をどの PlacementTarget に使ったか
```
...
これにより、最初は `10GBASE-SR generic x4` として出し、後から Connections で具体 SKU に置き換えられます。
...
| BOM の紐づけ表 | Materials / Assignments | Node と機材の割り当て |
...
| `BomItem` | Device assignment row | 既存名は残るが、意味は BOM ではなく機材行 |

現行の `BomItem` は device instance registry に近いですが、初期設計では在庫や資産ではなく「Product と Node を対応づける機材行」として使います。将来 `InventoryItem` を入れる場合も、`BomItem` を在庫に拡張するのではなく、別レイヤーからこの配置関係を参照します。
...
| 常に source jump できる | BOM、Assignments から Diagram / Connections の該当箇所へ移動できる |
...
Materials    Node への機材割り当てと部材定義を整える
BOM          必要部材リストを確認する
```
...
Assignments は現行 BOM が持っていた価値を移す場所です。Node に対して機材 Product を割り当てる表として編集します。module / cable は Link の接続仕様なので、Assignments へ混ぜず Connections で編集します。
...
│  ├─ search
│  ├─ product filter
│  ├─ status filter: unassigned / unplaced / placed
│  └─ add unplaced equipment
├─ Assignment table
│  ├─ Node
│  ├─ Equipment
│  ├─ status
...
この表は node assignment の編集 view です。保存上は `BomItem` が割り当て行、`Node` が図面上の配置を表します。Node 行で Product を選ぶと `paletteId` と bound Node の `productId` / spec snapshot を更新します。未配置機材を先に作った場合は、後から Node を選んで bind できます。
...
| `Tabs` / segmented control | Materials の Assignments / Library |
...
5. BOM view
6. propagation / diagnostics の強化
7. 将来 Inventory / Allocation / Usage
...
2. **Materials は Assignments と Library を持つ**
   - Product は device / module / cable / rack などの型。
   - Assignments は Node に対する機材割り当てをまとめて編集する view。
   - 個体管理ではない。
...
6. **資産管理は後で足せる形に残す**
   ...
   - `Allocation.target` は `PlacementTarget` を参照する。
   - その場合も `Product` / `NetworkGraph` / `Device Assignment` / `BOM` の基本構造は変えない。
