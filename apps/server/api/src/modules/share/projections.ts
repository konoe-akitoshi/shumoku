...
import type { Alert, MetricsData, MetricsMapping, NetworkGraph } from '@shumoku/core'
 ...
 */
export function publicTopologyContext(parsed: ShareParsedTopology) {
  return {
    id: parsed.id,
    name: parsed.name,
...
export function publicTopologyGraph(parsed: ShareParsedTopology) {
  return {
    id: parsed.id,
    name: parsed.name,
