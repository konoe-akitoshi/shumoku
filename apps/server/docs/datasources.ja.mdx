...
LLDP ネイバーにフォールバックした場合、**NCE の管理下にないネイバーもノードとして描画します**。WLAN だけを NCE で管理し、AP のアップリンク先スイッチは別管理、という構成がこれに当たります。AP↔スイッチのリンクは NCE だけが知っているトポロジなので、これを描かないと図が途切れます。管理外ノードにはネイバーが申告した MAC / chassisId / sysName が identity として載るため、NetBox や Zabbix 側の同一機器とマージされ、二重には出ません。デバイス種別は LLDP の申告内容から推定します（判別できない場合は L2 スイッチ）。識別情報を何も返さないネイバーは、マージできず実体を特定できないためリンクごと捨てます。

トークンは発行時のクライアント IP に束縛されるため、NAT やプロキシ経由でコントローラへアクセスする構成では送信元 IP が固定される必要があります。

リンクの使用率（ウェザーマップの色）は、デバイスのスループットカウンタをリンク管理が申告するポート速度で割って算出します。NCE のインターフェース別使用率フィールドは実機の AP では null が返るため、コントローラが使用率そのものを埋めている場合はそちらを優先し、埋めていない場合にこの計算値を使います。ポート速度が取れないリンクには使用率を出しません（色は「データなし」のグレーのまま）。

| フィールド | 説明 |
|-----------|------|
| NBI base URL | コントローラの NBI ベース URL（例: `https://nce.example.com:18002`） |
| Username | Open API Operator ロールを付与したサードパーティアカウント |
| Password | アカウントのパスワード |
| Site ID | 任意 — 1 サイト（UUID）に限定。空欄の場合はアカウントが参照できる全サイトをポーリング |
| Skip TLS verification | 任意 — 証明書検証をスキップ。オンプレのコントローラは Huawei の自己署名証明書（Platform Cert）を配ることが多いため、その場合に有効化（信頼できるネットワーク内のみ） |

サードパーティアカウントは NCE-Campus の **System > User Management** で Type を Third-party にして作成し、テナントビューの `Open API Operator` ロールを割り当ててください。参照系のみの monitor ロールでは NBI のトポロジ／パフォーマンス系エンドポイントが 403 を返します。
