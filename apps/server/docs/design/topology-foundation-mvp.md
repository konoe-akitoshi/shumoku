...
2. server の `/sources` で `network-scan` プラグインインスタンスを作成、seed と community を設定
...
### 2.3 SNMP-LLDP プラグイン（新規 `libs/plugins/network-scan`）
...
### 2.4 sysObjectID → vendor/model 同梱辞書（v1 で含める）

`libs/@shumoku/core/src/sys-object-id.ts` + JSON 同梱：

- [ ] `data/sys-object-id.json`: 公開データから抽出した
      `sysObjectID (or prefix) → { vendor, model? }` の辞書
- [ ] `lookupSysObjectID(oid: string): { vendor?, model? } | null`
- [ ] 識別フェーズで sysObjectID を引いたら辞書 lookup し、結果を node の
      `metadata.vendor` / `metadata.model` に埋める
- [ ] Element inspector で `model: Cisco Catalyst 9300 (sysObjectID 1.3.6.1.4.1.9.1.2030)`
      のように表示

**v1 に含めない**（v2 棚上げ）:
- カタログ全体の `discovery` capability 宣言（MIB / YANG / CLI family）
- 未知デバイスから「カタログ草稿」を起こす導線
- catalog series→model 継承との深い連携

→ user の要望「sysObjectID → model 引き」だけ満たす最小版。これで「unknown device」が
減り、Element inspector の情報量が大きく上がる。
...
- [ ] カタログ全体との統合（discovery capabilities 宣言、未知デバイスからカタログ草稿）
      — ただし sysObjectID → vendor/model 引きの**最小辞書同梱**は v1 で含める（§ 2.4）
- [ ] SNMPv3
...
GIVEN authored topology with 5 nodes
  AND network-scan source attached, configured with valid seed and community
...
sysObjectID 辞書（§ 2.4）は step 3 の SNMP プラグインスパイクと並行（半日程度の
追加コスト）。
...
4. `topology-foundation-snmp`: network-scan プラグイン
