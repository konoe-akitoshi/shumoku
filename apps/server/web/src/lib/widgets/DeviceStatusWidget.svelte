...
<WidgetWrapper title={config.title || 'Device Status'} {onRemove} onSettings={handleSettings}>
  <div class="h-full w-full relative overflow-hidden">
    ...
    {:else if error}
      <div class="h-full flex flex-col items-center justify-center text-danger gap-2">
        ...
        <button onclick={loadTopologyData} class="text-xs text-primary hover:underline">
          Retry
        </button>
      </div>
    ...
    {:else}
      ...
      >
        <svg
          class="w-full h-full transition-[viewBox] duration-300"
          {viewBox}
