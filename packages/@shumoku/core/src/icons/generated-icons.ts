 ...
 * @param vendor - Optional vendor name (e.g., "cisco", "juniper")
 */
export function getDeviceIcon(type?: DeviceType, _vendor?: string): string | undefined {
  if (!type) return undefined
  
  const iconKey = deviceTypeToIcon[type]
  if (!iconKey) return undefined
  
  // TODO: Add vendor-specific icon lookup when available
  // For now, always use default icons
