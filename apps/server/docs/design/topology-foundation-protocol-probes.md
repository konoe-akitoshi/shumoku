...
v1 の `snmp-lldp` プラグインは「識別 → IF-MIB walk → LLDP-MIB walk」を 1 つの
...
**B-3**: 既存 `libs/plugins/snmp-lldp/src/discover.ts` を分解 →
3 個の probe モジュールに：
...
**B-5**: `snmp-lldp` プラグイン → 廃止予定マーク。Network Discovery データソースは
