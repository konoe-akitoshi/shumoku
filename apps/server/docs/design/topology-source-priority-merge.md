...
## 3.5 詰めるべき穴（実装前に確定すること）

レビューで挙がった、実装段階で必ずぶつかる未確定点。**「priority(値の勝敗)」と
「identity(同一性判定)」は直交する別軸**であることを前提に分けて詰める。

### 穴1: 観測ソースの空値／null の扱い
§2 は「値を持つ最上位ソースが勝つ」とした。だが「持つ」の定義が曖昧。**人入力**の
空は §2.1 で弾くが、**観測ソース**が空文字 / null / 既定プレースホルダを返す場合
（例: NetBox が name 未設定で `""`）に、それを「値あり」として低 priority の実値を
潰すのか、「値なし」として読み飛ばすのか未定義。
→ 方針案: フィールド勝者選択は「**非 null かつ非空**」を「値あり」とする。空文字 /
空配列 / null は「持たない」扱いにして次 priority に譲る。要確定。

### 穴2: identity が食い違うと統合できず島に割れる
field 値の勝敗は priority で決まるが、**「そもそも同じノードか」(identity clustering)
に priority は効かない**。NetBox の mgmtIp と scan の mgmtIp が 1 つでも違えば別
クラスタ = 別ノード(島)に割れる。「統合したいのに島に割れて困る」が起きうる。
→ identity の信頼度（mgmtIp > chassisId > sysName）とソース優先度は別軸。v1 は
現行の identity clustering（いずれかのキー一致で同一）を維持。**手動マージ
（旧 Merge Config の `manual idMapping` 相当）を引き継ぐか**は §5.2 の宿題として残す
— 自動 clustering で拾えない食い違いは、手動マッピングが無いと統合できない。

### 穴3: 「人が触ったフィールド」の永続化形
「人 = 最優先ソース」と言ったが、人の寄与は **node 丸ごとではなくフィールド単位の
主張**（name だけ触った等）であるべき。現 Manual グラフは「Node」を保存する形。
**フィールド単位の主張をどうデータで持つか**（部分 Node / sparse field / 触った
フィールドの集合）が未設計。ここが曖昧だと tombstone 議論（触った vs 触ってない）が
ぶり返す。現 PR は「thin overlay Node（空ラベル sentinel + attachments）」で部分主張を
表現しているが、priority モデルでは「人ソースの寄与グラフ」も同じ表現でよいか、
フィールド集合を明示的に持つべきか要確定。

### 穴4: retraction（撤去）との相互作用
resolve には別途「N 回観測されなければ撤去」(retraction、未実装 TODO)がある。priority
マージとの相互作用が未記述：低 priority ソースから消えたノードを、高 priority ソース
（人 / 別観測）が保持し続けるのか。`absenceImpliesRetraction`（policy=disabled は撤去
しない）の既存ゲートと priority をどう噛ませるか、実装時に設計する。
