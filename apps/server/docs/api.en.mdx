...
compatibility `authenticated` flag. Anonymous requests remain anonymous and
cannot access workspace or management APIs. A future authenticated `viewer`
can read workspace resources but cannot mutate them or access administrator
surfaces.
