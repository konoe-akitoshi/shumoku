...
- config / options を、**ホストが汎用描画できるだけの表現力**を持つ schema で宣言。`PluginConfigProperty`
  を後方互換に強化（**実フォーム5種を全部描けることが要件**＝Phase 0 で机上証明する）:
...
- **§3.4.2 派生・表示専用情報（入力でない）**: grafana の webhook URL のように**作成後に生成される“出力”**は
  config 入力でない。プラグインが任意で **`getConnectionInfo(config): {label,value,copyable?}[]`** を返し、
  ホストが詳細画面に汎用表示する（schema の入力フィールドにしない）。これが無いと webhook URL 表示だけ
  ハードコード分岐が残る（#270 の残骸）。
...
- **(3) server**: timing-safe webhook guard（secret 定数時間比較＋payload は core の `validateAgainstSchema`）。
  webhook を受けるのは server でありプラグインでない。
...
0. **Phase 0 — schema 被覆の机上証明（煮詰める）**: バンドル5種の `configSchema`/`optionsSchema`＋grafana の
...
1. **Phase 1 — 共有ユーティリティ（§3.9、3層）**: core 純粋（`validateAgainstSchema`/severity/Alertmanager
   マッパ/`flattenObject`/`stampObserved`/`mapWithConcurrency`）＋ 新 `@shumoku/plugin-sdk`（`httpClient`/
   `paginate`）＋ server webhook guard。各々単体テスト付き。（C9 土台）
...
4. **Phase 4 — バンドル5種を共有 lib へ移行＋自己記述＋P0 修正**（plugin ごとにコミット）: Phase 0 の schema を
...
- **決定14**: コードを書く前に **Phase 0** で 5プラグインの schema を doc に書き起こし、`SchemaForm` 要件で
