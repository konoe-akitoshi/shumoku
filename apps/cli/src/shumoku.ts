...
import * as svg from '@shumoku/renderer-svg'
import * as htmlRenderer from '@shumoku/renderer-html'
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (format === 'html') {
      ...
      htmlRenderer.html.setIIFE(INTERACTIVE_IIFE)

      const sheets = await buildHierarchicalSheets(graph, layoutResult, layout)

      if (sheets.size > 1) {
        writeFileSync(outputPath, htmlRenderer.html.renderHierarchical(sheets), 'utf-8')
      } else {
        writeFileSync(outputPath, htmlRenderer.html.render(graph, layoutResult), 'utf-8')
      }
    } else if (format === 'png') {
      console.log('Rendering PNG...')
      const svgString = svg.svg.render(graph, layoutResult)
      ...
      writeFileSync(outputPath, svg.svg.render(graph, layoutResult), 'utf-8')
    }
