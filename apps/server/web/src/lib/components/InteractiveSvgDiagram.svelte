...
<script lang="ts">
  ...
  function setupInteractivity() {
    ...
    function closestInteractive(
      target: EventTarget | null,
    ):
      | { type: 'node'; el: SVGGElement; id: string }
      | { type: 'subgraph'; el: SVGGElement; id: string }
      | { type: 'link'; el: SVGGElement; id: string }
      | null {
      const el = (target as Element)?.closest?.(
        'g.node, g.subgraph, g.link-group',
      ) as SVGGElement | null
      if (!el) return null
      if (el.classList.contains('node'))
        return { type: 'node', el, id: el.getAttribute('data-id') || '' }
      if (el.classList.contains('subgraph'))
        return { type: 'subgraph', el, id: el.getAttribute('data-id') || '' }
      if (el.classList.contains('link-group'))
        return { type: 'link', el, id: el.getAttribute('data-link-id') || '' }
      return null
    }
...
<div class="diagram-container" bind:this={container} on:pointerdown={onPointerDown}>
  ...
  {:else}
    ...
    <div class="svg-wrapper" bind:this={svgWrapper}>{@html svgContent}</div>
  {/if}

  <!-- Tooltip -->
  {#if tooltipVisible}
    <div class="tooltip" style="left: {tooltipX}px; top: {tooltipY}px;">{@html tooltipContent}</div>
  {/if}
