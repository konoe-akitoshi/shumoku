...
- `Materials`: このプロジェクトで使う機材・部材の定義と、機材行と Node の紐づけ
...
| Materials | 標準機材、SKU、型番、機材行と Node の紐づけをまとめて整えたい | Product を登録し、機材行を Node に割り当てる |
...
flowchart LR
  ...
  A[Materials / Assignments<br/>機材行を Node へ割り当て]
...
Materials / Assignments では、現行 BOM が持っていた「機材行と Node の紐づけ」を表で編集します。これは KiCad の symbol instance と footprint / part を対応づける操作に近いです。module / cable は接続仕様の一部なので、Materials へ寄せず Connections で編集します。
...
flowchart TB
  ...
  Materials[Materials<br/>Library / Assignments]
  Library[Library<br/>Product / SKU / spec]
  Assignments[Assignments<br/>Device row ↔ Node]
  BOM[BOM<br/>必要部材リスト]
...
トップレベルは `Diagram` / `Connections` / `Materials` / `BOM` の 4 ページにします。`Materials` はこのプロジェクトで使う機材・部材を扱う workbench で、内部に `Library` / `Assignments` を持ちます。`BOM` は Materials の下位ではなく、設計全体から導出される確認ページとして独立させます。
...
- `Materials / Assignments` は機材行と Node の紐づけをまとめて編集する。
...
| Materials / Assignments | `BomItem[]`, `Node.productId` | 機材行と Node の紐づけ表 |
...
| Device Assignment | 機材行と Node の割り当て | `BomItem[]`、bound Node の `productId` | module / cable の接続仕様 |
...
Device Assignment は現行 BOM が持っていた「機材行と Node の対応」を正しい名前で扱う層です。ここが KiCad の schematic symbol と footprint / part を対応づける操作に相当します。
...
Product       = 型
Node / Link   = 設計インスタンス
Assignment    = 初期実装では機材行と Node の対応
...
| BOM の紐づけ表 | Materials / Assignments | 機材行と Node の割り当て |
...
| まとめて直す場所を持つ | Materials / Assignments で機材行と Node の未割当を表で一括修正できる |
...
Assignments は現行 BOM が持っていた価値を移す場所です。機材行と Node の紐づけを表で編集します。module / cable は Link の接続仕様なので、Assignments へ混ぜず Connections で編集します。
...
この表は device assignment の編集 view です。保存上は `BomItem` が機材行、`Node` が図面上の配置を表します。セルで Product を選ぶと機材行の `paletteId` と bound Node の `productId` / spec snapshot を更新し、Node を選ぶと機材行を既存 Node に bind します。
...
| `Tabs` / segmented control | Materials の Library / Assignments |
...
2. **Materials は Library と Assignments を持つ**
   - Product は device / module / cable / rack などの型。
   - Assignments は機材行と Node の紐づけをまとめて編集する view。
   - 個体管理ではない。
