...
| `SHUMOKU_PROXY_AUTH_DEFAULT_ROLE` | ロールが解決できないときの既定role（`viewer`/`user`/`admin`） | `admin` |
...
`DEMO_MODE`はサンプルデータ投入だけを行い、アクセス制御は変更しません。公開デモは
訪問者ごとに隔離された使い捨てコンテナを払い出し、インスタンス固有の管理者Secretを
注入して通常のログインフローを使う構成にします。通常サーバーの未認証リクエストを
アプリケーションroleへ変換して公開しないでください。
...
有効化すると、Shumokuは各リクエストの `SHUMOKU_PROXY_AUTH_USER_HEADER`
（既定 `X-Auth-Request-User`、無ければ `SHUMOKU_PROXY_AUTH_EMAIL_HEADER`）を
認証済みユーザーのsubjectとして扱います。roleは、`SHUMOKU_PROXY_AUTH_ROLE_HEADER`
が設定されていればその値を `SHUMOKU_PROXY_AUTH_ROLE_MAP`（例
`net-admins:admin,viewers:viewer`）で解決し、解決できなければ
`SHUMOKU_PROXY_AUTH_DEFAULT_ROLE`（既定 `admin`）を適用します。この認証は
ステートレスで、Shumokuのセッションcookieやローカルログインは使いません。
...
oauth2-proxy を使う場合は `--set-xauthrequest=true` を付けると既定ヘッダー名と
一致します。ローカルログイン用の初回管理者Secret（`SHUMOKU_BOOTSTRAP_ADMIN_PASSWORD*`）は
引き続き設定してください（プロキシ迂回時の破れ窓対策）。
