...
> この文書は旧レイアウト構成の設計記録を含む。本文の libavoid とそのファイル参照は現行仕様ではない。
> 現在の `route-edges.ts` は端点情報を生成し、描画側が曲線を描く。
> 画面の操作は [Diagram](../pages/diagram.md)、今後の設計は [auto-layout-redesign.md](./auto-layout-redesign.md) を参照。
