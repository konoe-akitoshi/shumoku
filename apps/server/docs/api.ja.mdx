...
プロキシ認証と復旧手順は[認証ガイド](api-access.guide.ja.md)と[SSO導入手順](installation.guide.ja.md)を参照してください。プロキシ認証中はローカルのログイン・初期設定・パスワード変更を無効化し、`authMethod: proxy`を返します。
...
リモートのHTTP/HTTPS originへ送信しません。本番環境ではローカルセッションまたはプロキシ認証を利用します。
プロキシ認証中は開発用credentialを無視します。credentialはAPI実行中のみgitignore済み・所有者限定のファイルへ
保存され、終了時に削除されます。
...
ローカルセッション・プロキシの身元・開発Bearerのいずれかと`workspace:read`権限が必要です。
...
| GET | `/api/topologies/:id/export?format=svg\|png\|html` | トポロジーをエクスポート |
| GET | `/api/topologies/:id/sources` | 紐付けデータソース一覧 |
| POST | `/api/topologies/:id/sources` | データソースを紐付け |

エクスポート結果は添付ファイルとして返されます。SVG と PNG は指定した `sheet`
（省略時はルートシート）を出力し、PNG では `0.25` から `4` の `scale` も指定できます。
HTML は全シートのナビゲーションを含む、単体で動作するインタラクティブな文書です。
...
非推奨の`publicDemo` fieldはAPI互換性のため残しますが、常に`false`であり、認証modeを
有効化・報告しません。未認証リクエストはanonymousのままで、workspace APIや管理APIへ
アクセスできません。認証済みviewerはworkspaceを読み取れますが、変更操作や
管理者向けsurfaceは利用できません。
...
`workspace:read`を持つ身元だけが接続できます。browser Originを検証し、HTTPと共通の方針で
ローカルsessionまたはプロキシの身元を解決します。接続済みsocketは切断までprincipalを
保持し、ロール変更は即時反映されません。過大なmessageは拒否します。
