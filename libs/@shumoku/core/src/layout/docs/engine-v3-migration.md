...
## 方向: 単一エンジンへ収束（2エンジンを恒久化しない）

**到達点は「レイアウト入口は1つ（`computeNetworkLayout`）」**。現状の
`shouldUseComposite` による *隠れたトップレベル2分岐*（composite vs flat-tree）は
**移行用の足場であって最終形ではない**。同じ製品が、データのヒューリスティックで
気づかぬうちに全く違う品質の図を出すのは設計負債である（実例: NetBox トポロジが
`metadata.location` を1つも持たないだけで旧 flat-tree に落ちていた — ゾーン情報は
あるのにノードへ転写していなかっただけ）。

最終形:

- 入口は composite パイプライン1本。いま flat-tree にフォールバックしている入力は
  **composite 内部の縮退ケース**として扱う:
  - **zone source の連鎖** `location > subgraph > 構造(betweenness/role-tier)` —
    ゾーン情報が無いグラフでも composite がゾーンを合成して動く。
  - **小規模/手描き** — 1ゾーン＋tidy 配置として composite 内で素直に処理する。tidy-tree は
    *composite 内部の配置プリミティブ*として再利用する（トップレベルの別エンジンには戻さない）。
- no-location / 小規模フィクスチャで invariant 通過＋品質 OK になった時点で
  **既定を composite に切替え、`shouldUseComposite` ゲートと flat-tree トップレベル分岐を
  撤去する**（= 撤去基準）。

収束を解錠する要石（child issues 化）:

1. **zone 合成フォールバック**（`location → subgraph → 構造`）— NetBox(subgraph持ち)・無印グラフを composite に乗せる
2. **root/apex の汎用決定** — 制約D の deferred（現状 location 由来 apex）を解消
3. **小規模対応＋性能** — 全グラフのデフォルトに耐える速度（現状 ~563ノードで分オーダー）

中間策（収束まで）: どのエンジンが走ったかを UI/メタに明示し、プラグイン境界で
`metadata.location` を出して（例: netbox）フォールバック率を下げる。
...
| A8 | `route-edges.ts` lane offset / bus | B2 ルータが既定になったら fallback に格下げ。**収束時にトップレベルの flat-tree 分岐は撤去**し、bezier+lane と tidy 配置は composite **内部**の縮退プリミティブ（小規模/1ゾーン）として残す（= 別エンジンではなく1エンジンの一部） | — |
...
6. **P5: B5 レンダラー意味文法 + 単一エンジンへの収束** — 要石(zone合成・root決定・小規模/性能)が
   揃い no-location/小規模フィクスチャが合格したら、**既定を composite に切替え、`shouldUseComposite`
   ゲートと flat-tree トップレベル分岐を撤去**する（「方向」節の撤去基準）。「2エンジン共存で止める」判断はしない
